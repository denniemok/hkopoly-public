...
| 語言 | 路徑 | 網站 |
|------|------|------|
| 英文 | `cards/en/cards.json` | [hkopoly.com](https://hkopoly.com) |
| 繁體中文（香港） | `cards/zh-HK/cards.json` | [zh.hkopoly.com](https://zh.hkopoly.com) |

英文版同中文版係**兩個唔同嘅伺服器**，房間同玩家唔互通。

卡牌**邏輯**（`action`、`target`、`amount` 等）喺各語言版本應保持一致；只有 **`text`**（同描述入面嘅金額用字）需要翻譯。修改卡牌時，除非改動只適用單一語言，否則請同時更新**兩個語言**嘅檔案。
...
- 卡牌文字與邏輯 → `cards/<locale>/cards.json`
- 棋盤格子與規則 → `maps/<locale>/<map-id>.json`
...
修改 `move` 卡的 `target` 時，請參考 [`maps/EDIT_MAP_GUIDE.zh-HK.md`](../maps/EDIT_MAP_GUIDE.zh-HK.md) 中的棋盤佈局，確保 `id` 指向預期的格子類型。

---

## 設計檢查清單

- [ ] 已更新 `cards/en/cards.json` 及／或 `cards/zh-HK/cards.json`
- [ ] 每張卡都有 `text` 和 `action`
...
1. 編輯 `cards/en/cards.json` 及 `cards/zh-HK/cards.json`（或你支援的語言）
2. 參考本指南確認欄位與動作類型正確
